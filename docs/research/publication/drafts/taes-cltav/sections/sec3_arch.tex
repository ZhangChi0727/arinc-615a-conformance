...
\begin{figure}[!ht]
\centering
\begin{tikzpicture}[font=\footnotesize,box/.style={draw,align=center,inner sep=3pt,text width=0.92\columnwidth}]
...
\draw[->] (sess) -- (adp);
\draw[->] ([xshift=-3pt]adp.north) -- ([xshift=-3pt]iut.south) node[midway,left,font=\scriptsize] {stimulus};
\draw[->] ([xshift=3pt]iut.south) -- ([xshift=3pt]adp.north) node[midway,right,font=\scriptsize] {raw record};
\draw[->] ([xshift=-3pt]adp.south) -- ([xshift=-3pt]sess.north) node[midway,left,font=\scriptsize] {raw record};
\draw[->] ([xshift=-3pt]sess.south) -- ([xshift=-3pt]interp.north);
\draw[->] ([xshift=3pt]interp.north) -- ([xshift=3pt]sess.south) node[midway,right,font=\scriptsize] {update};
\draw[->] (interp) -- (filt);
\draw[->] (truth) -- (filt);
\end{tikzpicture}
