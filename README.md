...
- Deliver an observable timed EFSM for UPLOAD and INFORMATION with payload-enabled accept/reject, visual TFTP-write LUR direction, WAIT not-before retry, obligation-level traces and the Attachment 4 retry equation.
...
- 交付 UPLOAD 与 INFORMATION 的可观测 timed EFSM，含 payload 使能的接受／拒绝、目视 TFTP 写的 LUR 方向、WAIT 不得早于重试、义务级追踪和附件 4 重试方程。
