...
FIG-CL-TAV-05 must show the no-stop return from S10 to S1 as well as the
retry back-edge. A later JRE re-render may replace the export; it is not a deferred delivery
gate for this increment. The typeset algorithm package (ALG-CLTAV-01 plus the
three core procedures ALG-CLTAV-05/06/07 in the main text and the S2/S6/S7--S8
coordination wrappers ALG-CLTAV-02/03/04 with the contract appendix, per
CR-2026-014 / DD-036) is copied to
`artifacts/publications/cltav/ALG-CLTAV-01.pdf`. The paper Algorithm caption
number is separate from repository id ALG-CLTAV-01. Chinese mapping uses stable
step labels S0–S10, not volatile typeset line numbers, protocol states, or time indices.
...
本环境 `java` 崩溃（`0xC0000409`）。FIG-CL-TAV-01／05／09 的读者 SVG 为等价节点／边导出：XML 有效、PlantUML 源放在 CDATA `desc`、控制流与禁止真值虚线均为可见 `line`。它们不是第二套架构权威。图 05 必须同时画出 S10 未停止时返回 S1 的正常循环，以及重试回边。后继可用 JRE 重渲染可以替换该导出，但不是本增量的延期交付门。排版算法包（ALG-CLTAV-01 及正文三个核心过程 ALG-CLTAV-05/06/07、S2／S6／S7--S8 协调包装 ALG-CLTAV-02/03/04 与契约附录，见 CR-2026-014／DD-036）已复制到 `artifacts/publications/cltav/ALG-CLTAV-01.pdf`。论文 Algorithm 编号与仓库 ID ALG-CLTAV-01 分开。中文对应使用稳定步骤标签 S0–S10，不引用易变排版行号、协议状态或时间索引。
