...
New protocol CRS rows are **not** produced in this increment by renaming `DEFERRED-FIND-M9` or `DEFERRED-DOWNLOAD-M9`. The ledger [`../../../configs/research/cltav_protocol_source_audit.json`](../../../configs/research/cltav_protocol_source_audit.json) lists current coverage units that must be re-read against 615A-3 before any requirement text is written. Batch template fill is forbidden. Tool software requirements stay out of this PR.
...
本增量**不**靠把 `DEFERRED-FIND-M9` 或 `DEFERRED-DOWNLOAD-M9` 改名来产生新协议 CRS 行。清单 [`../../../configs/research/cltav_protocol_source_audit.json`](../../../configs/research/cltav_protocol_source_audit.json) 列出必须对照 615A-3 重读的当前 coverage 单元，然后才能写需求正文。禁止批量套模板。工具软件需求不属于本 PR。
