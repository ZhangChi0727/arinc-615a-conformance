...
- Record FIND 78/78, DOWNLOAD 365/365 and AFDX appendix 13/13 source-unit reread candidates against 615A-3. Media Defined and Operator Defined DOWNLOAD stay separate. AFDX stays a conditional deployment and does not activate the current Compliant instance. Bound CRS rows stay deferred. 664/665/RFC body audits remain pending. This is not independent architecture, mathematical or RG approval.
...
- Write bilingual CRS rows from the recorded FIND, DOWNLOAD and AFDX-appendix candidates. Complete 665-5, 664P2/P3/P7 and RFC applicability audits. Do not batch-rename DEFERRED-* labels or generate CRS rows from templates. Software Ethernet tests do not prove AFDX hardware.
...
- 已对 FIND 78/78、DOWNLOAD 365/365 与 AFDX 附录 13/13 条对照 615A-3 记录来源重读候选。Media Defined 与 Operator Defined DOWNLOAD 保持分开。AFDX 仍为条件化部署，不激活当前 Compliant 实例。绑定 CRS 行仍延期。664／665／RFC 正文审计仍待进行。这不是独立架构、数学或 RG 批准。
...
- 按已记录的 FIND、DOWNLOAD 与 AFDX 附录候选撰写双语 CRS 行。完成 665-5、664P2／P3／P7 与 RFC 适用性审计。不得把 DEFERRED-* 标签批量改名，也不得按模板生成 CRS 行。普通以太网软件试验不能证明 AFDX 硬件保证。
