...
- 修正 CL-TAV 呈现（F-C，评审轮 R36／R37／R38）：正文承载总体算法（ALG-CLTAV-01）与三个核心机制（选择 ALG-CLTAV-05、时序观测 ALG-CLTAV-06、历史更新 ALG-CLTAV-07）；附录承载 S2／S6／S7-S8 协调包装（ALG-CLTAV-02/03/04）、完整类型记录、责任表与未实现内核登记。接口使用显式类型记录与字段访问；后端缺口先分支再投影；时序内核检查非空测量域与区分开／闭上界；S9 守卫是可执行条件且后继状态总定义；所有 Resolution 变体在分支前采用返回上下文，历史版本 fail-closed 校验。受限可执行调用生产模型与可执行组合见证检查编排。候选更新、停止、资源、重试与后继摘要语义均不变。
