...
- Correct the CL-TAV presentation (F-C, review round R36): the main text carries the top-level algorithm (ALG-CLTAV-01) and the three core mechanisms (selection ALG-CLTAV-05, timed observation ALG-CLTAV-06, history update ALG-CLTAV-07); the appendix carries the S2/S6/S7-S8 coordination wrappers (ALG-CLTAV-02/03/04), typed records and the unresolved-kernel register. Interfaces use explicit typed returns; a bounded executable-call production model and an executable composition witness check the new orchestration. No candidate-update, stop, resource, retry or successor-summary semantics change.
...
- 修正 CL-TAV 呈现（F-C，评审轮 R36）：正文承载总体算法（ALG-CLTAV-01）与三个核心机制（选择 ALG-CLTAV-05、时序观测 ALG-CLTAV-06、历史更新 ALG-CLTAV-07）；附录承载 S2／S6／S7-S8 协调包装（ALG-CLTAV-02/03/04）、类型记录与未实现内核登记。接口使用显式类型返回；受限可执行调用生产模型与可执行组合见证检查新编排。候选更新、停止、资源、重试与后继摘要语义均不变。
