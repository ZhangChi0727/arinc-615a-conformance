...
- Unify strictly-reducing then Prep selection, recovery eligibility, exclusive stop priority and row-level source-audit identity. This is not independent architecture, mathematical or RG approval.
...
- Close architecture REWORK items, then continue FIND/DOWNLOAD source-unit audit on this branch. Do not batch-rename DEFERRED-* labels or generate CRS rows from templates.
...
- 统一严格缩小后再 Prep 的选择、恢复资格、互斥停止优先级和来源审计逐行身份。这不是独立架构、数学或 RG 批准。
...
- 关闭架构 REWORK 项后，在本分支继续 FIND／DOWNLOAD 来源单元审计。不得把 DEFERRED-* 标签批量改名，也不得按模板生成 CRS 行。
