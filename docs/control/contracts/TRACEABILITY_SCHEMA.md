...
追踪关系是多对多关系：`rho_RT` 连接 CRS 与 TP，`rho_TV` 连接 TP 与 VC，`rho_RM` 连接义务与状态、转移、数据/时钟守卫、不变量、复位和时序目标，`rho_VF` 连接 VC 与故障算子，`rho_VE` 连接执行与证据。每条时序义务必须追踪到触发/响应/取消/替代谓词、关联键、配对/并发策略、端点包含性、界限、时钟启动/复位、误差预算 ID/版本、时序分区、稳健 oracle 和证据字段。缺失、悬空、退役或版本不匹配关系必须显式失败。
...
每个适用需求至少有一个 `rho_RT`，每个需求义务有充分且已评审的 VC，每个 TP 至少有一个可执行 VC，每个 VC 引用受控需求和模型目标。时序义务还必须连接触发/响应/取消/替代、关联与配对/并发、端点包含性、界限、时钟启动/复位、误差预算 ID/版本、时序分区、稳健 oracle 和证据字段。悬空、退役或版本不匹配 ID 失败；基础/扩展关系可区分；覆盖报告不得删除缺失链接。
