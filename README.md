...
- Write FIND bilingual CRS into M1-CANDIDATE-3 from recorded REQUIRED/OPTIONAL reread candidates (31 rows; commentary/examples stay non-normative). Bound M2 does not model FIND. DOWNLOAD 365/365 and AFDX appendix 13/13 remain deferred. Media Defined and Operator Defined DOWNLOAD stay separate. AFDX does not activate the current Compliant instance. 664/665/RFC body audits remain pending. This is not independent architecture, mathematical or RG approval.
...
- Write bilingual CRS rows from the recorded DOWNLOAD and AFDX-appendix candidates. Complete 665-5, 664P2/P3/P7 and RFC applicability audits. Do not batch-rename remaining DEFERRED-* labels or generate CRS rows from templates. Software Ethernet tests do not prove AFDX hardware.
...
- 已按已记录的 REQUIRED／OPTIONAL 重读候选把 FIND 双语 CRS 写入 M1-CANDIDATE-3（31 行；评注／示例保持非规范）。绑定 M2 不建模 FIND。DOWNLOAD 365/365 与 AFDX 附录 13/13 仍延期。Media Defined 与 Operator Defined DOWNLOAD 保持分开。AFDX 不激活当前 Compliant 实例。664／665／RFC 正文审计仍待进行。这不是独立架构、数学或 RG 批准。
...
- 按已记录的 DOWNLOAD 与 AFDX 附录候选撰写双语 CRS 行。完成 665-5、664P2／P3／P7 与 RFC 适用性审计。不得把仍延期的 DEFERRED-* 标签批量改名，也不得按模板生成 CRS 行。普通以太网软件试验不能证明 AFDX 硬件保证。
