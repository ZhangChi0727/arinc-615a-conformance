...
Current bound package still has 2796 coverage units and 384 requirements. FIND 78/78 source units have candidate reread notes; bound rows stay `DEFERRED-FIND-M9`. DOWNLOAD (`DEFERRED-DOWNLOAD-M9`) and AFDX appendix units stay unread. Ledger: `configs/research/cltav_protocol_source_audit.json`. Do not batch-rename deferred labels. Tool software requirements are out of this PR.
...
当前绑定包仍为 2796 条 coverage、384 条需求。FIND 78/78 条来源单元已有候选重读记录；绑定行仍为 `DEFERRED-FIND-M9`。DOWNLOAD（`DEFERRED-DOWNLOAD-M9`）和 AFDX 附录单元尚未重读。清单：`configs/research/cltav_protocol_source_audit.json`。不得批量改名延期标签。工具软件需求不属于本 PR。
