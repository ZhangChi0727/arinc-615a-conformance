...
- Correct Table 6.4.4-1 to LUR and Table 6.4.5-1 to LUS; align the LUR write triad to DATA-LOADER WRQ, TARGET-HARDWARE ACK and DATA-LOADER DATA; keep LUI field-table predicates as a named gap. This PR exit is a bounded M2 baseline, not a development-ready CRS. Add source-explicit 615A→665 and TFTP-option refinements used by the model; keep AFDX, FIND, 645 and P3 deviations deferred.
...
- 将表 6.4.4-1 纠正为 LUR、表 6.4.5-1 纠正为 LUS；将 LUR 写三元组对齐为 DATA-LOADER WRQ、TARGET-HARDWARE ACK、DATA-LOADER DATA；LUI 字段表谓词作为具名缺口。本 PR 出口是有界 M2 基线，不是开发就绪 CRS。为模型实际使用的对象增加 615A→665 与 TFTP 选项精化；FIND、AFDX、645 与 P3 偏差保持延期。
