...
FIND, DOWNLOAD and AFDX-appendix bilingual CRS continue in `M1-CANDIDATE-11`
(currently 2936 coverage / 646 requirements after RR-CLTAV-2026-011 TFTP
end-condition rebind; FIND abort does not waive the 2 s / 3 s clocks). Remaining 645 binding and
...
FIND、DOWNLOAD 与 AFDX 附录双语 CRS 继续在 `M1-CANDIDATE-11`（当前 2936 条 coverage／646 条需求，经 RR-CLTAV-2026-011 结束条件重绑；FIND 中止不豁免 2 秒／3 秒时钟）。645 绑定与 CRS-M1-00519／664-4 **不**在本 PR 闭合。清单 [`../../../configs/research/cltav_protocol_source_audit.json`](../../../configs/research/cltav_protocol_source_audit.json) 仍保存 FIND 78/78、DOWNLOAD 365/365 与 AFDX 附录 13/13 条重读记录。`profileScope.instanceBoundOperations` 仍为 UPLOAD／INFORMATION；`researchExpandedOperations` 为 DOWNLOAD／FIND。Media Defined 与 Operator Defined DOWNLOAD 保持分开。AFDX 仍为条件化部署，不激活当前 Compliant 实例。绑定 M2 不执行 FIND、DOWNLOAD 或 AFDX；FIND 时序只进入观察目录。禁止批量套模板。工具软件需求不属于本 PR。ARINC 645 本地已取得但未绑定。
