...
- Record FIND source-unit reread candidates for all 78 DEFERRED-FIND-M9 coverage units against 615A-3 Attachment 3 and §5.3.3. Bound CRS rows stay deferred. DOWNLOAD and AFDX appendix units remain unread. This is not independent architecture, mathematical or RG approval.
...
- Continue Media Defined and Operator Defined DOWNLOAD source-unit reread on this branch. FIND 78/78 candidate reread is recorded; bound DEFERRED-FIND-M9 rows are unchanged. Do not batch-rename DEFERRED-* labels or generate CRS rows from templates.
...
- 已对全部 78 条 DEFERRED-FIND-M9 coverage 对照 615A-3 附件 3 与 §5.3.3 记录 FIND 来源重读候选。绑定 CRS 行仍延期。DOWNLOAD 与 AFDX 附录单元尚未重读。这不是独立架构、数学或 RG 批准。
...
- 在本分支继续 Media Defined 与 Operator Defined DOWNLOAD 来源单元重读。FIND 78/78 条候选重读已记录；绑定的 DEFERRED-FIND-M9 行未改。不得把 DEFERRED-* 标签批量改名，也不得按模板生成 CRS 行。
